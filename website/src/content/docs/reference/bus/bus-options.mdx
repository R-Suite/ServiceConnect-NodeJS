...
- **`defaultRequestTimeout`** (optional) — `number` (ms). Falls back to `DEFAULT_REQUEST_TIMEOUT_MS` (`10_000`). The per-call `RequestOptions.timeoutMs` always wins; this is the floor when you omit it.
