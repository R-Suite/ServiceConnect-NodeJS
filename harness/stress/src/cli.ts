...
export function parseCli(argv: readonly string[]): CliOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--chaos-interval':
        chaosIntervalSec = readInt(readValue(argv, ++i, '--chaos-interval'), '--chaos-interval');
        break;
      case '--chaos-downtime':
        chaosDowntimeSec = readInt(readValue(argv, ++i, '--chaos-downtime'), '--chaos-downtime');
        break;
      case '--broker':
      ...
      case '--memory-budget-mb':
        memoryBudgetMb = readInt(readValue(argv, ++i, '--memory-budget-mb'), '--memory-budget-mb');
        break;
      case '--report-dir':
