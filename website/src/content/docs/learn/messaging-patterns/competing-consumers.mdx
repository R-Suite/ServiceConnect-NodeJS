...
const bus = createBus({
  ...
  queue: { name: 'orders', prefetch: 16 },
});
```

There is no special API — you are running the same service code in multiple processes, each with the same `queue.name`. The broker handles dispatch. `prefetch: 16` tells RabbitMQ how many unacknowledged messages this consumer is willing to hold at once.
...
- `prefetch` (passed via `queue.prefetch`) controls how many unacknowledged messages a single consumer will hold at once. Higher values = more parallelism per process, at the cost of less even load distribution — a fast worker can grab a chunk of the queue and slow peers will idle. A modest value (8–32) is a good default; tune to your handler's latency profile.
