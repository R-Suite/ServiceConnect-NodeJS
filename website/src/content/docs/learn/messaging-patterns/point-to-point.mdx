...
await bus.send<ChargeCard>('payments', 'ChargeCard', {
  orderId: 'o-1',
  total: 49.99,
});
```

The first argument — `'payments'` — is the **endpoint**: the name of the queue you are sending to. The producer does not need to know whether one process or twenty are consuming that queue; RabbitMQ handles dispatch. The receiving service registers a handler for `ChargeCard` exactly the same way it would for any other typed message.
