...
// v3 — fan-out request to many endpoints
const quotes = await bus.sendRequestMulti<GetQuote, Quote>(
  'GetQuote',
  { correlationId: 'c-1', sku: 'abc' },
  { endpoints: ['pricing-a', 'pricing-b', 'pricing-c'], timeoutMs: 5000 },
);
console.log(quotes.length, 'quotes received');
```

For the broadcast (publish-then-collect-replies) case, v3 still exposes `bus.publishRequest` for symmetry with v1, with a per-reply callback. Most callers should prefer the promise-returning forms — they compose with `await`, `Promise.all`, and `AbortController` in ways the callback form cannot.
