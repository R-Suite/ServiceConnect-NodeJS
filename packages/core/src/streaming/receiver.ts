...
export class StreamReceiver<T extends Message> implements AsyncIterable<T> {
  ...
  push(rec: ReceivedChunk<T>): void {
    ...
    if (rec.faulted !== undefined) {
      ...
      for (const res of this.resolvers.splice(0)) res({ value: undefined as never, done: true });
      return;
    }
    if (rec.isEnd) {
      this.endSequenceNumber = rec.sequenceNumber;
  ...
  [Symbol.asyncIterator](): AsyncIterator<T> {
    return {
      next: () => {
        ...
        if (this.endSequenceNumber !== undefined && this.nextExpected > this.endSequenceNumber) {
