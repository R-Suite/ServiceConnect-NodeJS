...
- Records the `messaging.process.duration` histogram and increments the `messaging.client.consumed.messages` counter (tagged with `messaging.outcome` — `success`, `error`, or `retry`) before ending the span.
