...
- **Registration order does not matter** for parents — declare children first or last; the registry stores the parent list verbatim and the producer applies it (publishing to parent exchanges) when messages are published.
...
- Stamp the message's type identity onto every outbound message (the `TypeName` wire header).
