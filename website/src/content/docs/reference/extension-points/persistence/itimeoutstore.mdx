...
- **`delete(id)`** — remove a timeout. Called after the bus has fired the timeout into the saga successfully, or when a saga marks itself complete. Safe to no-op if the row no longer exists.
