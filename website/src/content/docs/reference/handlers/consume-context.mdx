...
- **`logger`** — a child [`Logger`](/ServiceConnect-NodeJS/reference/configuration/options/#logger) with `messageType`, `messageId`, and `correlationId` bound; every log line emitted through it inherits those fields.
