...
export function telemetryConsumeWrapper(
  ...
  return (cb: ConsumeCallback): ConsumeCallback =>
    async (envelope, signal) => {
      ...
      const messageType = typeof headers.messageType === 'string' ? headers.messageType : 'unknown';
      ...
      try {
        const result = await context.with(trace.setSpan(parent, span), () => cb(envelope, signal));
        if (result.success) {
