...
export function streaming(alphaQueue: string, betaQueue: string): PatternFlow {
  ...
  return {
    ...
    async register(a: Bus, b: Bus): Promise<void> {
      ...
      a.registerMessage<Chunk>(typeName).handleStream<Chunk>(typeName, makeHandler(pendingAlpha));
      b.registerMessage<Chunk>(typeName).handleStream<Chunk>(typeName, makeHandler(pendingBeta));
    },
