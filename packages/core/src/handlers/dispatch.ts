...
export function createDispatcher(deps: DispatcherDeps): ConsumeCallback {
    return async function dispatch(
        ...
        // Forward a routing slip (if present) to its next destination. Must run on EVERY
        // success-classified consume exit — saga, aggregator, no-handler, and the normal handler
        // path — or a routed message that is processed by one of those branches is acked with its
        // remaining itinerary silently dropped. forwardRoutingSlipIfPresent no-ops when the message
        // carries no slip or when handlerSucceeded is false, so this is safe on all paths.
        const maybeForward = async (handlerSucceeded: boolean): Promise<void> => {
            if (!deps.routingForward) return;
            try {
        ...
        try {
            const action = await deps.pipelines.before.execute(envelope, {
                signal,
                logger: deps.logger,
            });
        ...
        if (deps.sagaBranch) {
            ...
            if (sagaOutcome.ran && sagaOutcome.result) {
                await maybeForward(sagaOutcome.result.success === true);
                await runAfterSafe(deps, envelope, signal);
        ...
        if (deps.aggregatorBranch) {
            ...
            if (aggOutcome.ran && aggOutcome.result) {
                await maybeForward(aggOutcome.result.success === true);
                await runAfterSafe(deps, envelope, signal);
        ...
        if (resolved.length === 0) {
            ...
            // message is not dropped at a passive hop.
            await maybeForward(true);
            await runAfterSafe(deps, envelope, signal);
            return { success: true, notHandled: true, terminalFailure: false };
        }
