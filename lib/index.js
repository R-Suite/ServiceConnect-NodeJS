...
var Bus = exports.Bus = function (_EventEmitter) {
    ...
    _createClass(Bus, [{
        ...
        value: function _consumeMessage(message, headers, type) {
            var promises = void 0;
            try {
                promises = [].concat(_toConsumableArray(this._processHandlers(message, headers, type)), _toConsumableArray(this._processRequestReplies(message, headers, type)));
            } catch (e) {
                throw {
                    exception: e,
                    success: false
        ...
        value: function _processHandlers(message, headers, type) {
            ...
            if (handlers.length > 0) {
                ...
                handlers.map(function (handler) {
                    ...
                    var promise = handler(message, headers, type, replyCallback);
        ...
        value: function _processRequestReplies(message, headers, type) {
            ...
            if (headers["ResponseMessageId"]) {
                ...
                if (configuration) {
                    var promise = configuration.callback(message, type, headers);
                    if (promise !== null && promise !== undefined) {
                        promises.push(promise);
                    }
