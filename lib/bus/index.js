...
class Bus {
     ...
     */
    async publishRequest(type, message, callback, expected = null, timeout = null, headers = {}) {
        const shouldPublish = await this.filterManager.executeOutgoing(this.config.filters.outgoing, message, headers, type, this);
        if (!shouldPublish) {
            return;
        }
        ...
        this.requestReplyManager.registerRequest(messageId, expectedCount, callback, timeoutMs);
        headers.RequestMessageId = createMessageId(messageId);
        if (this.core.client) {
            await this.core.client.publish(type, message, headers);
        }
