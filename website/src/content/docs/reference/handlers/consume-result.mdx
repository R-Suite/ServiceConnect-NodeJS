...
- **`notHandled`** — `boolean`. `true` if the message type is not registered at all, or is registered but has no handler resolved at this endpoint. Transports typically log and ack `notHandled` envelopes (the message reached a queue we own, but nothing here cares about it).
...
| Dispatch outcome | `success` | `notHandled` | `error` | `terminalFailure` |
|---|---|---|---|---|
